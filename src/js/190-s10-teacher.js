  ...
  ENTER.s10 = () => {
      ...
      <div class="tmain" id="tmain">
        ...
        <div class="tpage" data-pane="quick">
          ...
          <div class="tcheck" id="tcheck">
            ...
            ${exeReady ? `<span class="tchk warn"><i></i><b>${updKind === 'app' ? '앱' : '실행기'}</b><span>새 버전 ${exeReady} 준비됨</span><button class="tlink2" data-act="exerestart">지금 다시 시작</button></span>` : ''}
          </div>
          <div class="tquick">
    ...
    el.querySelectorAll('[data-act]').forEach(b => b.addEventListener('click', () => {
      ...
      else if (a === 'bkpaste') { pop(); const box = el.querySelector('#bk-box'), ta = el.querySelector('#bk-text'); box.style.display = ''; ta.value = ''; el.querySelector('#bk-apply').style.display = ''; el.querySelector('#bk-msg').textContent = '코드를 붙여 넣고 [이 코드로 복구]'; ta.focus(); (navigator.clipboard && navigator.clipboard.readText ? navigator.clipboard.readText() : Promise.reject()).then(t => { if (/YK[12]\./.test(t) && !ta.value) { ta.value = t.trim(); el.querySelector('#bk-msg').textContent = '클립보드의 코드를 붙여 넣었어요 — [이 코드로 복구]'; } }).catch(() => {}); }
